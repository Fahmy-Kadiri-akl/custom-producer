...
func (t *Target) Type() string { return "cloudflare_token" }
...
func (t *Target) Rotate(ctx context.Context, req *types.RotateRequest) (*types.RotateResponse, error) {
	...
	newToken, err := client.CreateToken(ctx, p.TokenName, p.Policies, p.Condition)
	if err != nil {
		return nil, fmt.Errorf("create token: %w", err)
	}
	log.Info().Str("new_id", newToken.ID).Str("name", p.TokenName).Msg("new Cloudflare token created")
	if p.TokenID != "" {
		if err := client.DeleteToken(ctx, p.TokenID); err != nil {
			log.Warn().Err(err).Str("old_id", p.TokenID).Msg("failed to delete old Cloudflare token")
		} else {
			log.Info().Str("old_id", p.TokenID).Msg("old Cloudflare token deleted")
		}
	}
	p.TokenID = newToken.ID
	p.Token = newToken.Value
