...
func (t *Target) Type() string { return "sendgrid_key" }
...
func (t *Target) Rotate(ctx context.Context, req *types.RotateRequest) (*types.RotateResponse, error) {
	...
	client := NewClient(p.AdminKey)
	newKey, err := client.CreateKey(ctx, p.KeyName, p.Scopes)
	if err != nil {
		return nil, fmt.Errorf("create key: %w", err)
	}
	log.Info().Str("new_id", newKey.ID).Str("name", p.KeyName).Msg("new SendGrid key created")
	if p.KeyID != "" {
		if err := client.DeleteKey(ctx, p.KeyID); err != nil {
			log.Warn().Err(err).Str("old_id", p.KeyID).Msg("failed to delete old SendGrid key")
		} else {
			log.Info().Str("old_id", p.KeyID).Msg("old SendGrid key deleted")
		}
	}
	p.KeyID = newKey.ID
	p.Key = newKey.Key
