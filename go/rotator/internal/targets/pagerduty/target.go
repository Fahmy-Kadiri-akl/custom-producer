...
func (t *Target) Type() string { return "pagerduty_key" }
...
func (t *Target) Rotate(ctx context.Context, req *types.RotateRequest) (*types.RotateResponse, error) {
	...
	newKey, err := client.CreateKey(ctx, p.KeyName, kt)
	if err != nil {
		return nil, fmt.Errorf("create key: %w", err)
	}
	log.Info().Str("new_id", newKey.ID).Msg("new PagerDuty key created")
	if p.KeyID != "" {
		if err := client.DeleteKey(ctx, p.KeyID); err != nil {
			log.Warn().Err(err).Str("old_id", p.KeyID).Msg("failed to delete old PagerDuty key")
		} else {
			log.Info().Str("old_id", p.KeyID).Msg("old PagerDuty key deleted")
		}
	}
	p.KeyID = newKey.ID
	p.Key = newKey.Key
